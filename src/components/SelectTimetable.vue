<template>
  <v-container fluid>
      ...
      >
        ...
        <v-col class="mx-auto">
          ...
          <v-row justify="center">
            <v-col cols="6" class="mx-auto" v-for="(year) in ttsInfo.yearList" :key="year">
              <h2 class="font-weight-regular">Année {{year}} :</h2>
              <v-col
                class="pt-2 pb-1 mx-auto"
                v-for="(timetable, i) in ttsInfo.ttsByYear[year]"
              ...
              >
                ...
                >
                  <v-btn
                    ...
                    :color="colorList[i]"
                    @click="selectEDT(timetable.adeResources)"
                    dark
                  >{{timetable.nameTT}}</v-btn>
                </router-link>
              </v-col>
            </v-col>
          </v-row>
    ...
    >
      <v-btn
        :class="{'subtitle-1 pl-12 pr-12': $vuetify.breakpoint.lg || $vuetify.breakpoint.xl}"
        @click="requestPage('SelectUniv')"
      >Retour à la sélection de l'établissement</v-btn>
    </v-col>
  </v-container>
</template>
