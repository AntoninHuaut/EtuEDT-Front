import { roomDetailsRequest, roomEventsRequest, timetableDetailsRequest, timetableEventsRequest } from "@/api/api_requests";
...
export function buildResourceDetailsRequest(context: IResourceRequestContext) {
    ...
    return timetableDetailsRequest(context.numUniv, context.groupId ?? 0, context.adeResources);
}
...
export function buildResourceEventsRequest(context: IResourceRequestContext) {
    ...
    return timetableEventsRequest(context.numUniv, context.groupId ?? 0, context.adeResources);
}